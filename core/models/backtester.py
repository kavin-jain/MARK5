    ...
    def run_simulation(self, df: pd.DataFrame, signals: pd.Series, symbol: str = 'UNKNOWN') -> Tuple[pd.DataFrame, dict]:
        ...
        for i in range(1, len(df)):
            ...
            if position != 0:
                ...
                # Path B: Momentum Trend-Following Exit Logic
                # Remove Time Stops and Profit Targets. Use Trailing Stop.
                
                if position > 0: # Long
                    # Trailing stop: based on HIGHEST CLOSE since entry, not current close
                    # This prevents the stop jumping backwards on ATR spikes
                    if not hasattr(self, '_highest_close') or self._highest_close == 0.0:
                        self._highest_close = entry_price
                    self._highest_close = max(self._highest_close, curr_close)
                    trail_level = self._highest_close - (curr_atr * self.atr_multiplier)
                    if current_trailing_stop == 0.0:
                        current_trailing_stop = entry_price - (curr_atr * self.atr_multiplier)
                    else:
                        current_trailing_stop = max(current_trailing_stop, trail_level)
                        
                    stop_price = current_trailing_stop
                    
                    # Scenario A: Gap Down below Trailing Stop
                    ...
                    if current_trailing_stop == 0.0:
                        ...
                        # Trail the stop down
                        trail_level = curr_close + (curr_atr * self.atr_multiplier)
                        current_trailing_stop = min(current_trailing_stop, trail_level)
                        
                    stop_price = current_trailing_stop
                    
                    # Scenario A: Gap Up above Trailing Stop
                    if curr_open >= stop_price:
                         sl_hit = True
                         exit_price = curr_open * (1 + self.slippage)
                         reason = "SL_GAP"
                ...
                if sl_hit:
                    ...
                    entry_price = 0.0
                    current_trailing_stop = 0.0
                    self._highest_close = 0.0  # reset peak tracker
                    # 1-bar cooldown after hard stop — prevents same-bar re-entry only
            ...
            if cooldown > 0:
                ...
                # Position Sizing (Hybrid Regime/Confidence Scaling)
                # Fetch confidence if available (passed as a column in df, or default to 0.55)
                conf = df['confidence'].iloc[i-1] if 'confidence' in df.columns else 0.55
                
                # Dynamic Risk Scaling — tiered allocation by conviction
                # Floor raised to 25% to ensure each trade meaningfully captures moves.
                # 20% was too low: 1 trade at 20% yields 0.4% on a 2% stock move,
                # which can't overcome transaction costs or beat a lightly-leveraged B&H.
                if conf >= 0.65:
                    dyn_risk  = self.risk_per_trade * 2.0  # 3.0% risk
                    max_alloc = 0.40  # Tier 1 (high conviction breakout)
                elif conf >= 0.60:
                    dyn_risk  = self.risk_per_trade * 1.5  # 2.25% risk
                    max_alloc = 0.35  # Tier 2 (medium conviction)
                else:
                    dyn_risk  = self.risk_per_trade        # 1.5% risk
                    max_alloc = 0.25  # Tier 3: raised from 20% → 25% (continuation entries)
                    
                # ALLOCATION-DRIVEN SIZING FOR MOMENTUM 
                # (Bypasses distance-based starvation to match B&H leverage)
                max_pos_value = equity * max_alloc
            ...
            mtm_equity = equity
            if position != 0:
                unrealized_pnl = (curr_close - entry_price) * position
