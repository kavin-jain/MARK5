...
| K7 | **Public ownership-flow signal** (institutional FII+DII accumulation) | ❌ KILL | **[H]** | **Confirmed on FULL DEEP data (2026-06-07): 198 stocks, ~32q to 2018, real disclosure dates.** Δ-Institutions IC(1y)=−0.025; FII-buyers vs sellers −0.6pp (no edge). Winner study: corr(pre-run Δinst, run size)=−0.204 — top winners ALL PSU/rail (IRFC/BSE/HUDCO/RVNL/SAIL/NBCC/IRCON) with institutions FLAT/SELLING before; institutions CHASE not lead. Priced in by disclosure. Paid Trendlyne would NOT have helped. See §4 I1. | **RE-TESTED 2026-08-09 on the correct statistic (`ownership_signal_study.py::report_orthogonal_ic`, 754 names, 16 cross-sections, median 290 names each).** The original verdict was reached with a POOLED Spearman across all tickers and dates — that is not an IC, which is cross-sectional by construction — and on RAW IC, the §4 Group B error. Both redone: cross-sectional raw IC −0.0035 (t −0.15), **residual IC after the momentum composite is projected out −0.0179 (t −0.70)**, correlation to the existing composite +0.183. Pre-registered bar was residual IC ≥ 0.03 AND t ≥ 3.0; it fails both, and the sign is negative. **KILL now stands on evidence rather than on a mis-specified statistic.** Caveat recorded: the panel carried a freshness warning (259/1350 tickers ending >7d before the window end), so the most recent cross-sections are the weakest — the result does not hinge on them.
...
| K46 | **Does the composite ranking beat the field it picks from?** (head-to-head, not IC) | ✅ KEEP, qualified | **[M]** | `score_meaning_study.py` (2026-08-09, 300-name eligible universe, 74 monthly evaluation dates 2019-11→2026-07, 126-bar forward): the picked top-20 beat the equal-weight eligible field in **49 of 74 periods (66%)**, median edge **+5.29pp per 6 months**, worst period −9.61pp, best +28.65pp. Monotonic by score band: median forward return +6.8% (bottom 20%) → +12.8% (top 10%) → +13.6% (the 20 actually bought). **t = 2.0 on ~12 independent windows — under this project's 3.0 bar.** Qualification is the sample, not the sign: a ~5pp edge cannot be proven at t≥3 with six years of overlapping data, so this is evidence the ranking works and NOT proof. Caveats recorded in the report: overlapping windows, gross of costs and tax, single names rather than the portfolio, and survivorship in the cached universe (delisted names absent, so figures are optimistic). **A third of top-scored names still lost money.** |
| K47 | **Instrument-type contamination of the equity universe** (ETFs ranked as stocks) | ✅ FIXED | **[H]** | Name matching failed three times: `_is_etf` catches *BEES/*ETF only, so MON100 and MAFANG were added by hand, then HDFCGOLD/HDFCSILVER/TATSILV/LIQUIDCASE appeared the same way. **LIQUIDCASE is a cash fund and scored 100th percentile on BOTH low-vol and stability** — inverse-vol sizing gives the steadiest name the most money, and a crash inverting momentum is exactly when it would reach the top. Root fix: classify by ISIN (equity = INE…, fund units = INF…, 329 of 2,685 bhavcopy symbols), applied once in `discover_tickers()`. Universe 462→456. |
| K48 | **Sector cap silently disabled for unlabelled names** | ✅ FIXED | **[H]** | `sector_map.json` was built by hand once (500 names, 2026-07-22) and never refreshed, so every company listed since was unlabelled — and unlabelled names are treated as their own sector and **escape the cap entirely**. Gaps were concentrated where they mattered: 30/300 eligible unlabelled but **7 of the top 20**, three of those capital-goods names. `fetch_sector_map.py` pulls NSE's own index-constituent files (752 names) and runs before every rebalance; coverage 270/300 → 296/300, top-20 gaps 7 → 1. Labels are never invented: an unmapped name stays outside the cap and `/sector` names it explicitly. |
| K49 | **Kronos (shiyu-coder) K-line foundation model — re-proposed 2026-08-09** | ❌ DECLINED without re-test | **[H]** | Already covered by K9, which integrated Kronos/Chronos at ≤10% weight and found **no proven OOS return edge**. The reason is structural, not empirical (§4j): Kronos consumes CANDLESTICKS, so it is another function of the same OHLCV series the momentum composite is built from — it lies in the span already occupied. *Recombining price data cannot escape the span it already occupies.* That is why K1–K9 all failed and why the one signal that worked (`deliv_chg`) came from data price cannot express. **Fair-test caveat kept on the record:** K9 sits in Group B, whose verdicts were reached on RAW IC rather than IC orthogonal to the composite, so the verdict is method-limited. If ever retested the ONLY admissible design is residual IC after projecting out the existing composite, pre-registered at t ≥ 3.0. Expected value at ₹5L capital: a *good* new signal is worth ~₹5,440/yr total (K40), against GPU inference every rebalance. |
...
| K40 | **Rebalance cadence re-swept on the deployed config, 19y** (21/42/63/126/189/252/378/504 bars) | ❌ KILL | **[H]** | `cadence_and_filter_sweep.py` (2026-08-09): nothing clears the pre-registered bar — best challenger (42d) wins **12/17** rolling 3-yr windows against a required 13, and its worst window is no better (−19.9% vs −19.8%). Deployed 126d posts the LOWEST full-period CAGR of the eight (+12.05%) yet is not displaced, which is the point: K21 found 126d BEST on 2016-2026 and this finds it WORST on 2007-2026. **The instability is the finding** — cadence is not a reliable lever and picking a winner is picking noise. |
| K44 | **Universe width by rank** (top 100/200/300/500/800 by liquidity) | ❌ WITHDRAWN — measurement error | **[H]** | Same script. top-800 appeared to beat top-300 by **+4.75pp CAGR** and 14/17 windows, monotonic across the range — a textbook-looking result. It is an artefact: the 500th most liquid NSE name traded **₹0.01cr/day in 2013** and ₹11.50cr/day in 2026, so a rank cut means something different at each end of the window and widening it buys untradeable microcaps that the engine fills at closing prices with flat slippage. **Caught before it was recommended.** Mandate §0 in its purest form. |
| K42 | **Universe width by absolute rupee liquidity floor** (₹1/3/5/10/25cr/day median turnover) | ❌ KILL | **[H]** | Same script — the time-consistent version of K29, since a rupee floor means the same thing in every year and the universe grows on its own as the market deepens. No floor beats the top-300 rank cut: best (₹1cr) wins **9/17**, and it has a WORSE worst window (−22.3% vs −19.8%). The inherited top-300 default survives, now on evidence rather than inheritance. |
| K43 | **`n_hold` 20 → 60** (staged for the Jan-2027 rebalance) | ❌ DECLINED | **[H]** | IR does peak near 60 (0.365 → 0.433) but **t = 1.90**, under the §5 bar of 3.0 and under even the conventional 2.0 — the configs are statistically indistinguishable. It also collides with the four-sleeve allocation: 25% equity across 60 names is ~₹2,166/position at this NAV and six current holdings cost more than that per SHARE. Unaffordable names are silently dropped, and affordability tracks share price, which is arbitrary — an undisclosed tilt toward low-priced shares. The breadth sweep runs in scale-free NAV units where fractional shares are implicit, so it could not have seen this. Viable above ~₹25 lakh. |
| K45 | **1/N ensemble over the 5 factor-weight schemes** (staged for Jan-2027) | ❌ DECLINED | **[H]** | `ensemble_feasibility.py` (2026-08-09). The case FOR it stands and is epistemic, not financial: averaging all five schemes removes config *selection*, so PBO stops applying to the choice, priced at −1.92pp CAGR for ~1.2pp of drawdown. It is simply not buyable here. Averaging five top-20 books is a breadth expansion in disguise — names all five agree on keep a full weight, a name only one scheme picks gets a fifth of one. Result: **29 target names, 3 round to zero whole shares, smallest position ₹894** vs a single config's 19 names, 0 zeros, ₹4,197 smallest. ₹894 does not buy one share of SUNPHARMA (₹1,945), ASIANPAINT (₹2,735) or ADANIENT (₹3,020). Same wall as K43, same undisclosed tilt toward low-priced shares. Revisit above ~₹25 lakh. |
